...
const courses = [
  { name: 'Ciência da Computação', city: 'Bauru · Presidente Prudente · Rio Claro · São José do Rio Preto', area: 'Tecnologia', description: 'Computação, dados e criação de soluções para problemas reais.' },
  { name: 'Direito', city: 'Franca', area: 'Humanas', description: 'Formação pública para atuar com justiça, direitos e políticas.' },
  { name: 'Engenharia Agronômica', city: 'Botucatu · Dracena · Ilha Solteira · Jaboticabal · Registro', area: 'Exatas', description: 'Tecnologia, produção e sustentabilidade no campo.' },
  { name: 'Medicina', city: 'Botucatu', area: 'Saúde', description: 'Cuidado, ciência e compromisso com o SUS.' },
  { name: 'Arquitetura e Urbanismo', city: 'Bauru · Presidente Prudente', area: 'Exatas', description: 'Projetar cidades, espaços e futuros mais habitáveis.' },
  { name: 'Ciências Biológicas', city: 'Assis · Bauru · Botucatu · Ilha Solteira · Jaboticabal · Rio Claro · São José do Rio Preto · São Vicente', area: 'Saúde', description: 'Vida, biodiversidade e pesquisa em diferentes territórios.' },
  { name: 'Administração', city: 'Jaboticabal · Tupã', area: 'Humanas', description: 'Gestão com impacto público, social e econômico.' },
  { name: 'Design', city: 'Bauru', area: 'Artes', description: 'Criatividade, linguagem visual e inovação aplicada.' },
];
...
export default function Home() {
  ...
  return <main>
    ...
    <section id="cursos" className="course-section">
      ...
      <div className="course-grid">{results.map((course) => <article className="course-card" key={course.name}><p>{course.area}</p><h3>{course.name}</h3><span className="city">{course.city}</span><p className="description">{course.description}</p><a href={guideUrl} target="_blank" rel="noreferrer">Conheça o curso <span>↗</span></a></article>)}</div>
